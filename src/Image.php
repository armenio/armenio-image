<?php  
// All credits go to CodeIgniter
namespace Armenio\Image;
...
class Image 
{
	...
	public function initialize($props = []) {
		...
		if ($this->new_image == '') {
			...
			if (mb_strpos($this->new_image, '/') === false && mb_strpos($this->new_image, '\\') === false) {
				$this->dest_folder = $this->source_folder;
		...
		if( $this->crop ) {
			$cX = $this->orig_width/$this->width;
			$cY = $this->orig_height/$this->height;
			
			if($cX > $cY){
				$this->x_axis = ($this->orig_width-($this->width*$cY))/2;
				$this->orig_width = $this->orig_width - ($this->x_axis*2);
			}else{
				$this->y_axis = ($this->orig_height-($this->height*$cX))/2;
				$this->orig_height = $this->orig_height - ($this->y_axis*2);
			}

			$this->dst_width = $this->width;
			$this->dst_height = $this->height;
		} elseif ( $this->maintain_ratio === true ){
			$this->image_reproportion();

			if( $this->fit_to_canvas === true ) {
				$this->dst_x_axis = floor(($this->width - $this->dst_width) / 2);
				$this->dst_y_axis = floor(($this->height - $this->dst_height) / 2);
			}else{
				$this->width = $this->dst_width;
				$this->height = $this->dst_height;
			}
		} else {
			$this->dst_width = $this->width;
			$this->dst_height = $this->height;
		...
		// Set the quality
		$this->quality = (int) trim(str_replace('%', '', $this->quality));
		...
		if (mb_strlen($this->wm_font_color) == 6) {
			$this->wm_font_color = '#'.$this->wm_font_color;
		}

		if (mb_strlen($this->wm_shadow_color) == 6) {
			$this->wm_shadow_color = '#'.$this->wm_shadow_color;
		}

		if ( ! empty($this->wm_overlay_path) ) {
			$this->wm_overlay_path = str_replace("\\", "/", realpath($this->wm_overlay_path));
		}

		if ( ! empty($this->wm_shadow_color) ) {
			$this->wm_use_drop_shadow = true;
		}

		if ( ! empty($this->wm_font_path) ) {
	 ...
	 * Image Resize
	 *
	 * This is a wrapper public function that chooses the proper
	 * resize public function based on the protocol specified
	 *
	 * @access	public
	 * @return	bool
	 */
	 ...
	 *
	 * This is a wrapper public function that chooses the proper
	 * cropping public function based on the protocol specified
	 *
	 * @access	public
	 * @return	bool
	 */
	 ...
	 *
	 * This is a wrapper public function that chooses the proper
	 * rotation public function based on the protocol specified
	 *
	 * @access	public
	 * @return	bool
	 */
	 ...
	 *
	 * This public function will resize or crop
	 *
	 * @access	public
	 * @param	string
	 * @return	bool
...
*/
		...
		if ($this->image_type == 3) // png we can actually preserve transparency
		{
			...
			$color = imagecolorallocate($dst_img, $this->bg_color[0], $this->bg_color[1], $this->bg_color[2]);
			imagefill($dst_img, 0, 0, $color);
		}

		$copy($dst_img, $src_img, $this->dst_x_axis, $this->dst_y_axis, $this->x_axis, $this->y_axis, $this->dst_width, $this->dst_height, $this->orig_width, $this->orig_height);
	 ...
	 *
	 * This public function will resize, crop or rotate
	 *
	 * @access	public
	 * @param	string
	 * @return	bool
	 ...
	 *
	 * This public function will flip horizontal or vertical
	 *
	 * @access	public
	 * @return	bool
	 */
	 ...
	 * Image Watermark
	 *
	 * This is a wrapper public function that chooses the type
	 ...
	 *
	 * This public function lets us re-proportion the width/height
	...
	public function image_reproportion() {
		...
		$new_width	= ceil($this->height * $this->orig_width /$this->orig_height);
...
/*
		if( $this->fit_to_canvas === true ) {
			$this->dst_x_axis = floor(($this->width - $new_width) / 2);
			$this->dst_y_axis = floor(($this->height - $new_height) / 2);
		} else {
			$this->width = $new_width;
			$this->height = $new_height;
		}
*/
	 ...
	 *
	 * A helper public function that gets info about the file
	 *
	 * @access	public
	 * @param	string
	 * @return	mixed
	 ...
	 * Size calculator
	 *
	 * This public function takes a known width x height and
	 ...
	 *
	 * This is a helper public function that extracts the extension
	 * from the source_image.  This public function lets us deal with
